...
const getSingleRuleSection = ({ id, ...rule }) =>
  CardService.newCardSection()
    ...
    .addWidget(
      CardService.newKeyValue()
        .setTopLabel('Do this:')
        .setContent(`Auto-reply with "${rule[EMAIL.NAME]}"`)
        .setMultiline(true)
    )
    .addWidget(
...
const existingRulesSection = () => {
  const sections = [];
  fetchServiceRules(SERVICE.RESPONDER).forEach(rule => {
    sections.push(getSingleRuleSection(rule));
  });
  return sections;
};

const cardGmailRespond = message => {
  const card = CardService.newCardBuilder();
  card.setHeader(
    CardService.newCardHeader()
      .setTitle('Email Responder')
      .setSubtitle('Smart Auto-Replies for Gmail')
  );
...
export default cardGmailRespond;
